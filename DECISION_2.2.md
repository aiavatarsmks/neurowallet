...
## ✅ Safe foundation shipped (2026-07-18, flag OFF — needs no account)

The provider-agnostic, decision-neutral core is built and tested, inert until you
activate it:
- `lib/swap-quote.ts` — pure BigInt fee/slippage math + transparent breakdown
  (acceptance "наша маржа видна"); default markup **85 bps (~0.85%, plan target)**,
  overridable via `SWAP_MARKUP_BPS`; slippage default 1% / hard cap 5%; v1 chain
  set = EVM+SOL+TON (TRX/BTC out of scope, not faked). 9 tests.
- `POST /api/swap/quote` — read-only quote proxy, **hard-gated**: flag
  `NEXT_PUBLIC_SWAP_ENABLED` + `ONEINCH_API_KEY` + auth + rate-limit; returns 503
  while dark. **No execution/signing** (that needs the signer + explicit confirm,
  per invariant — added later). 8 tests.
- The 1inch HTTP call is isolated in `routerGrossOut()` and returns a stub until
  wired — marked **⚠️ VERIFY** against current 1inch Swap API docs + per-chain
  token-address resolution before enabling.

**To activate:** (1) create 1inch account → `ONEINCH_API_KEY` in Vercel, (2) set
`NEXT_PUBLIC_SWAP_ENABLED=true`, (3) confirm the markup %, (4) wire+verify
`routerGrossOut()`. Steps 1 and 3 are the only ones that need you.
