...
> **⚠️ Открытый custody-вопрос (ждёт решения Максима):** пользователи, онбордившиеся до 2026-06-27, держат BTC (P2PKH) и TON на старых путях деривации. На своём устройстве всё работает, но при **переимпорте seed** (recovery/новое устройство) приложение деривит новые пути → старые BTC/TON становятся невидимы/нетратимы через приложение (не потеряны — восстановимы во внешнем кошельке). Деривацию НЕ трогал. Анализ и варианты решения — в `LEGACY_DERIVATION_JUNE_USERS.md`.
