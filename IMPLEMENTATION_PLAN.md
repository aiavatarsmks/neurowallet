...
> **✅ Custody-вопрос закрыт (2026-07-04, accept + document):** адреса до 2026-06-27 используют старую derivation-схему; при переимпорте seed приложение переключается на новую и старые BTC/TON перестают быть видны (но восстановимы той же seed в кошельке со старой схемой). Подтверждено, что реальных активов на старых адресах нет → dual-derive/sweep не делаем. Зафиксировано как известное поведение в `KEY_MANAGEMENT.md`; разбор — `LEGACY_DERIVATION_JUNE_USERS.md`.
...
### ⭐ Корректировка по GTM-исследованию (08.07.2026)

Правки к Фазе 2 по новому исследованию роста/дистрибуции/монетизации Telegram-first кошелька. **При конфликте с задачами ниже — приоритет у этой секции.**

- **TON-first compliance (обязательно).** Telegram Blockchain Guidelines: мультичейн **разрешён внутри TON-based кошелька** (управление BTC/ETH/… в самом интерфейсе), но кошелёк должен быть **TON-based**, без линковки на внешние приложения для управления не-TON активами; эмиссия токенов — только на TON (мы токены не выпускаем). **TON Connect** — как рельса для TON-dApp (формально мы как pure-wallet освобождены, но добавляем к посуре). Позиционирование: **«TON-native multichain wallet with AI safety»**, а не «равноправный мультичейн». Аудит и детали — задача **2.10** / `COMPLIANCE_TG.md`. **Единственный канал дистрибуции — Telegram, риск бана недопустим.**
- **Рост — через продуктовые social loops, НЕ рекламу.** Бюджета на маркетинг нет; виральность встроена в продукт (claim-ссылки 2.8, request/gift money, referral после funded 2.9). **Swap/on-ramp — ПОСЛЕ социальных петель.**
- **AI как слой безопасности, не «умный ассистент».** Нейра подаётся как «кошелёк, который не даёт ошибиться» (симуляция, anti-poisoning, risk engine, объяснения), а не как «умный помощник».
- **Retention-таргеты D30:** 4–8% (все запуски) / 8–12% (создан кошелёк) / 10–18% (funded).
- **Промо-уведомления ≤ 1–2/нед** (транзакционные — без лимита; см. 2.4 preference center).
- **Ориентиры монетизации:** swap ~0.85%, on-ramp partner fee 0.5–1.25%, premium $6–15/мес; рассмотреть **Telegram Stars subscriptions**.

### 🔢 Новый порядок Фазы 2 (по GTM)

**2.6 → 2.8 → 2.4 (доделки) → 2.7 → 2.10 → 2.1 → 2.3 → 2.2 → 2.9.**
Логика: сначала социальные петли роста (2.6/2.8) и retention (2.4/2.7), затем compliance-хардненинг (2.10), потом seedless-онбординг (2.1) и revenue (2.3/2.2), referral (2.9) — последним, когда есть funded-действия для анти-фрода. Номера задач сохранены для трассируемости; **исполнять в этом порядке, а не по возрастанию номеров.**
...
- Default retail path: embedded MPC wallet (провайдер класса Coinbase Embedded / Dynamic / Web3Auth — выбрать по TON+Tron+Solana покрытию, custody-модели, цене; свести в сравнительную таблицу перед интеграцией и согласовать с Максимом). **Черновик сравнения готов — `EMBEDDED_WALLET_PROVIDERS.md`** (ключевой вывод: провайдеры с raw-подписью на обеих кривых secp256k1+ed25519 — Turnkey / Web3Auth — покрывают все 5 чейнов, включая TON, с переиспользованием нашего `lib/crypto`; нужен PoC-спайк перед решением).
...
Паттерн xRocket / Wallet in Telegram. **Главная встроенная петля роста.**
- Отправитель создаёт claim-ссылку (deeplink в Mini App) на сумму; получатель по ссылке → онбординг (если нет кошелька) → зачисление.
- Плюс **request money** («пришли мне X») и **gift/cheque** (подарочный чек).
- **Анти-фрод:** expiry ссылки, автo-возврат отправителю по истечении, лимиты на сумму/частоту, защита от двойного клейма (атомарное списание/зачисление, идемпотентность).
- Compliance: claim/зачисление в рамках TON-based кошелька; для не-TON активов — в интерфейсе, без внешних приложений.
- **Приёмка:** ссылка одноразовая и идемпотентная; истёкшая → возврат отправителю; невозможен двойной клейм; онбординг из клейма конвертит в funded-действие.

### 2.9 Referral — ТОЛЬКО после funded action (последним)

- Двусторонний реферал; награда начисляется **только после funded-действия** приглашённого (не за регистрацию/подключение).
- **Анти-фрод стек (обязателен):** device fingerprint, возраст Telegram-аккаунта, velocity rules, запрет self-referral / same-device, manual review всплесков.
- **Compliance:** НЕЛЬЗЯ награждать за подключение не-TON кошельков (прямой запрет гайдлайнов).
- **Приёмка:** награда не начисляется без funded-события; self/same-device отсекаются; всплеск уходит на ручной ревью.

### 2.10 Соответствие Telegram Blockchain Guidelines (рамка принята 08.07)

**Обязательно до масштабирования.** Единственный канал — Telegram. Полная
рамка и BotFather-тексты — в **`COMPLIANCE_TG.md`**.
- **Идентичность:** «TON-кошелёк с AI-защитой, внутри — управление BTC/ETH/SOL/TRX/USDT».
- **Тексты:** описание бота/Mini App (BotFather — готовые тексты в `COMPLIANCE_TG.md`, применяет Максим), онбординг, privacy/лендинг → TON-native.
- **Продукт:** Receive default = TON; акцент онбординга = TON; на главной TON-активы не ниже остальных. **Send-пикер НЕ трогаем** (USDT TRC-20 первым — utility).
- **TON Connect — ОТЛОЖЕН** (не сейчас): режим «no external connections» explicitly permitted для pure-wallet. **Пред-условие будущих экосистемных интеграций** (TON-dApp-интероп) — внедрять тогда.
- **Гигиена (подтверждено):** нет внешних ссылок на управление не-TON активами и каталогов dApp; block-explorer ссылки оставлены (информационные); 2.9 referral — награды только за funded-действия, не за подключение внешних кошельков.
- **Приёмка:** позиционирование TON-native применено (UI + BotFather); гигиена подтверждена; известные риски закрыты/приняты в `COMPLIANCE_TG.md`.

**Gate фазы 2:** социальные петли (claim-ссылки 2.8) дают органический рост; D30 retention в таргете (см. GTM-корректировку); compliance-аудит (2.10) пройден; ПОСЛЕ этого — revenue-события (swap/on-ramp 2.2/2.3).
