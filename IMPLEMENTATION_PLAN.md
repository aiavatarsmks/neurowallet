...
### 2.10 Аудит соответствия Telegram Blockchain Guidelines + TON Connect

**Обязательно до масштабирования.** Единственный канал — Telegram.
- Полный аудит Mini App против `core.telegram.org/bots/blockchain-guidelines` по первоисточнику; результат и чек-лист — **`COMPLIANCE_TG.md`**.
- Репозиционирование в **TON-native multichain wallet** (UI-формулировки, стор-описание, онбординг).
- Поддержка **TON Connect** (рельса для TON-dApp).
- Проверить серые зоны: внешние ссылки на не-TON эксплореры; отсутствие каталогов не-TON приложений; referral не награждает за не-TON кошельки.
- **Приёмка:** аудит зафиксирован в `COMPLIANCE_TG.md`; известные риски закрыты или явно приняты; TON Connect работает.
- **Ночь #2 (08.07):** предварительная проверка проведена — чёткого black-letter нарушения нет; главный риск — интерпретация «TON-based» (позиционирование). Детали в отчёте/`COMPLIANCE_TG.md` (создать).
